function [] = experiment_2D(count, iter, dt, K, sigma, beta, init_pos, init_vel, show_center, simulate_implicit, colors)
...
%
%   init_pos - Initial positions, it's a matrix with "count" rows and 2
%   columns (set to "[]" to skip this argument).
%
%   init_vel - Initial velocities, it's a matrix with "count" rows and 2
%   columns (set to "[]" to skip this argument).
%
...
a = figure("Name","Discrete simulation");
figure(a);
result_plot_2D(Pos, Vel, P_Center, C_Pos_1, V_Center, C_Vel_1, show_center, colors);

[Pos, C_Pos_2, P_Center, Vel, C_Vel_2, V_Center] = ode45_solver(iter*dt, K, sigma, beta, X, V);

b = figure("Name","Numerical (ode45) solution");
figure(b);
result_plot_2D(Pos, Vel, P_Center, C_Pos_2, V_Center, C_Vel_2, show_center, colors);
...
if simulate_implicit
    ...
    figure(c);
    result_plot_2D(Pos, Vel, P_Center, C_Pos_3, V_Center, C_Vel_3, show_center, colors);
    
    d = figure("Name","Solution comparation");
    figure(d);
    comparison_plot_2D(C_Pos_2,C_Vel_2,"Numerical (ode45) solution",[0,1,0],...
        C_Pos_1,C_Vel_1,"Discrete simulation",[1,0,0],...
