function [] = data_plot_3D (data, colors, plot_title, line_style, step_style, origin_style, data_legend)
...
% a 3D figure. This shows only positions/velocities and not time points.
%
...
%   dimension is 3 and third dimension is index of a time point 
...
%   individuals.
    ...
    for i = 1:count
        ...
        z_data = permute(data(i,3,:),[3 2 1]);

        % line
        if (data_legend == "None") || (i ~= 1)
        plot3(x_data,y_data,z_data,step_style,'Color',color)
        else
            legend('AutoUpdate','on')
            plot3(x_data,y_data,z_data,step_style,'Color',color,'DisplayName',data_legend)
            legend('AutoUpdate','off')
        end

        % timesteps
        plot3(x_data,y_data,z_data,line_style,'Color',color)
        
        % origin
        plot3(data(i,1,1),data(i,2,1),data(i,3,1),origin_style,'Color',color) 
    end
    hold off