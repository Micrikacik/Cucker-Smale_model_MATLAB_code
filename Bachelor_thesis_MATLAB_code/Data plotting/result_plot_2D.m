function [] = result_plot_2D(Pos, Vel, P_Center, C_Pos, V_Center, C_Vel, show_center, colors)
...
% Plots the result of the experiment_2D.
%
...
%   dimension is 2 and third dimension is timestep.
%
...
%   is 1, second dimension is 2 and third dimension is timestep.
%
...
nexttile
data_plot_2D(Pos,colors,"Absolute positions")
if show_center == true
    center_plot_2D(P_Center)
end

nexttile
data_plot_2D(C_Pos,colors,"Relative positions")

nexttile
data_plot_2D(Vel,colors,"Absolute velocities")



if (show_center == true) && (isequal(V_Center, 0) == false)
    center_plot_2D(V_Center)
end
...
if isequal(C_Vel, 0) == false
    nexttile
    data_plot_2D(C_Vel,colors,"Relative velocities")
end

function [] = center_plot_2D (m_center)
    hold on
    x_data = permute(m_center(1,1,:),[3 2 1]);
    y_data = permute(m_center(1,2,:),[3 2 1]);
    plot(x_data,y_data,['o','k'],'MarkerSize',10)
    hold off